...
def write_parquet_s3(uri, rows):
    with fsspec_open(uri, "wb") as f:
        pl.DataFrame(rows).write_parquet(f)


def test_approve_csv_multiple_local_files(tmp_path):
...
def test_approve_csv_multiple_s3_files(s3_uri):
...
def test_approve_jsonl_multiple_local_files(tmp_path):
...
def test_approve_jsonl_multiple_s3_files(s3_uri):
...
def test_approve_parquet_multiple_local_files(tmp_path):
...
def test_approve_parquet_multiple_s3_files(s3_uri):
