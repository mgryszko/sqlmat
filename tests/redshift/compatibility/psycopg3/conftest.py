...
def conn(redshift_env: RedshiftEnv) -> Generator[psycopg.Connection]:
    c = psycopg.connect(
...
@pytest.fixture
def src_table(conn: psycopg.Connection, registry: SchemaRegistry, schema: str) -> RedshiftTable:
...
@pytest.fixture
def tgt_table(conn: psycopg.Connection, registry: SchemaRegistry, schema: str) -> RedshiftTable:
    columns = [("user_id", "integer"), ("event_date", "date"), ("event_count", "integer")]
