from datetime import date

import pyathena
import pytest

from sqlmat import Executor, FullRefreshTableTransformation, IncrementalTableTransformation
from sqlmat.adapters import TARGET_TABLE_ALIAS, AthenaAdapter
from sqlmat.test import SchemaRegistry, Table


@pytest.fixture
def adapter(conn: pyathena.connection.Connection, s3_table_base_uri: str) -> AthenaAdapter:
    return AthenaAdapter(conn, s3_table_base_uri=s3_table_base_uri)


@pytest.fixture
def executor(adapter: AthenaAdapter) -> Executor:
    return Executor(adapter)


def test_full_refresh_templated(
    conn: pyathena.connection.Connection, executor: Executor, registry: SchemaRegistry, src_table: Table, tgt_table: Table
) -> None:
    src_table.insert([(1, date(2024, 1, 1), 5), (1, date(2024, 1, 2), 3), (2, date(2024, 1, 1), 7)])

    class TemplatedTransform(FullRefreshTableTransformation):
        target_schema = tgt_table.schema
        target_table = tgt_table.name
        sql = """
              select user_id, max(event_date) as event_date, sum(event_count) as event_count
              from {{ source_table }}
              group by user_id
              """

    executor.run(TemplatedTransform(), template_context={"source_table": src_table.qualified_name})

    tgt_table.assert_table_equals(
        [
            {"user_id": 1, "event_date": date(2024, 1, 2), "event_count": 8},
            {"user_id": 2, "event_date": date(2024, 1, 1), "event_count": 7},
        ],
        order_by=["user_id"],
    )


def test_full_refresh_non_templated(
    conn: pyathena.connection.Connection, executor: Executor, registry: SchemaRegistry, tgt_table: Table
) -> None:
    class NonTemplatedTransform(FullRefreshTableTransformation):
        target_schema = tgt_table.schema
        target_table = tgt_table.name
        sql = "select 42 as user_id, date '2024-01-01' as event_date, 100 as event_count"

    executor.run(NonTemplatedTransform())

    tgt_table.assert_table_equals([{"user_id": 42, "event_date": date(2024, 1, 1), "event_count": 100}])


def test_delete_insert_single_unique_key(
    conn: pyathena.connection.Connection, executor: Executor, registry: SchemaRegistry, src_table: Table, tgt_table: Table
) -> None:
    tgt_table.insert([(1, date(2024, 1, 1), 10), (2, date(2024, 1, 1), 20)])

    class DeleteInsertTransform(IncrementalTableTransformation):
        target_schema = tgt_table.schema
        target_table = tgt_table.name
        strategy = "delete_insert"
        unique_key = "user_id"
        sql = "select user_id, max(event_date) as event_date, sum(event_count) as event_count from {{ source_table }} group by user_id"

    src_table.insert([(2, date(2024, 1, 2), 25), (3, date(2024, 1, 3), 30)])
    executor.run(DeleteInsertTransform(), template_context={"source_table": src_table.qualified_name})

    tgt_table.assert_table_equals(
        [
            {"user_id": 1, "event_date": date(2024, 1, 1), "event_count": 10},
            {"user_id": 2, "event_date": date(2024, 1, 2), "event_count": 25},
            {"user_id": 3, "event_date": date(2024, 1, 3), "event_count": 30},
        ],
        order_by=["user_id"],
    )

    src_table.delete()
    src_table.insert([(3, date(2024, 1, 3), 35), (4, date(2024, 1, 4), 40)])
    executor.run(DeleteInsertTransform(), template_context={"source_table": src_table.qualified_name})

    tgt_table.assert_table_equals(
        [
            {"user_id": 1, "event_date": date(2024, 1, 1), "event_count": 10},
            {"user_id": 2, "event_date": date(2024, 1, 2), "event_count": 25},
            {"user_id": 3, "event_date": date(2024, 1, 3), "event_count": 35},
            {"user_id": 4, "event_date": date(2024, 1, 4), "event_count": 40},
        ],
        order_by=["user_id"],
    )


def test_delete_insert_composite_unique_key(
    conn: pyathena.connection.Connection, executor: Executor, registry: SchemaRegistry, src_table: Table, tgt_table: Table
) -> None:
    tgt_table.insert([(1, date(2024, 1, 1), 10), (2, date(2024, 1, 1), 20), (1, date(2024, 1, 2), 15), (2, date(2024, 1, 2), 25)])
    src_table.insert([(1, date(2024, 1, 2), 16), (2, date(2024, 1, 2), 26), (1, date(2024, 1, 3), 30), (2, date(2024, 1, 3), 35)])

    class DeleteInsertTransform(IncrementalTableTransformation):
        target_schema = tgt_table.schema
        target_table = tgt_table.name
        strategy = "delete_insert"
        unique_key = ["user_id", "event_date"]
        sql = "select user_id, event_date, event_count from {{ source_table }}"

    executor.run(DeleteInsertTransform(), template_context={"source_table": src_table.qualified_name})

    tgt_table.assert_table_equals(
        [
            {"user_id": 1, "event_date": date(2024, 1, 1), "event_count": 10},
            {"user_id": 1, "event_date": date(2024, 1, 2), "event_count": 16},
            {"user_id": 1, "event_date": date(2024, 1, 3), "event_count": 30},
            {"user_id": 2, "event_date": date(2024, 1, 1), "event_count": 20},
            {"user_id": 2, "event_date": date(2024, 1, 2), "event_count": 26},
            {"user_id": 2, "event_date": date(2024, 1, 3), "event_count": 35},
        ],
        order_by=["user_id", "event_date"],
    )

    src_table.delete()
    src_table.insert([(1, date(2024, 1, 3), 31), (2, date(2024, 1, 3), 36), (1, date(2024, 1, 4), 40), (2, date(2024, 1, 4), 45)])
    executor.run(DeleteInsertTransform(), template_context={"source_table": src_table.qualified_name})

    tgt_table.assert_table_equals(
        [
            {"user_id": 1, "event_date": date(2024, 1, 1), "event_count": 10},
            {"user_id": 1, "event_date": date(2024, 1, 2), "event_count": 16},
            {"user_id": 1, "event_date": date(2024, 1, 3), "event_count": 31},
            {"user_id": 1, "event_date": date(2024, 1, 4), "event_count": 40},
            {"user_id": 2, "event_date": date(2024, 1, 1), "event_count": 20},
            {"user_id": 2, "event_date": date(2024, 1, 2), "event_count": 26},
            {"user_id": 2, "event_date": date(2024, 1, 3), "event_count": 36},
            {"user_id": 2, "event_date": date(2024, 1, 4), "event_count": 45},
        ],
        order_by=["user_id", "event_date"],
    )


def test_delete_insert_with_incremental_predicates_single_string(
    conn: pyathena.connection.Connection, executor: Executor, registry: SchemaRegistry, src_table: Table, tgt_table: Table
) -> None:
    src_table.insert([(2, date(2024, 1, 2), 25), (3, date(2024, 1, 2), 30)])
    tgt_table.insert([(1, date(2024, 1, 1), 10), (2, date(2024, 1, 1), 16), (3, date(2024, 1, 1), 5)])

    class DeleteInsertTransform(IncrementalTableTransformation):
        target_schema = tgt_table.schema
        target_table = tgt_table.name
        strategy = "delete_insert"
        unique_key = "user_id"
        incremental_predicates = f"{TARGET_TABLE_ALIAS}.event_count > 15"
        sql = "select user_id, event_date, event_count from {{ source_table }}"

    executor.run(DeleteInsertTransform(), template_context={"source_table": src_table.qualified_name})

    tgt_table.assert_table_equals(
        [
            {"user_id": 1, "event_date": date(2024, 1, 1), "event_count": 10},
            {"user_id": 2, "event_date": date(2024, 1, 2), "event_count": 25},
            {"user_id": 3, "event_date": date(2024, 1, 1), "event_count": 5},
            {"user_id": 3, "event_date": date(2024, 1, 2), "event_count": 30},
        ],
        order_by=["user_id", "event_date"],
    )


def test_delete_insert_with_incremental_predicates_list(
    conn: pyathena.connection.Connection, executor: Executor, registry: SchemaRegistry, src_table: Table, tgt_table: Table
) -> None:
    src_table.insert([(1, date(2024, 1, 2), 16), (2, date(2024, 1, 2), 26)])
    tgt_table.insert([(1, date(2024, 1, 1), 5), (1, date(2024, 1, 2), 15), (2, date(2024, 1, 1), 8), (2, date(2024, 1, 2), 15)])

    class DeleteInsertTransform(IncrementalTableTransformation):
        target_schema = tgt_table.schema
        target_table = tgt_table.name
        strategy = "delete_insert"
        unique_key = ["user_id", "event_date"]
        incremental_predicates = [f"{TARGET_TABLE_ALIAS}.event_date >= date '2024-01-02'", f"{TARGET_TABLE_ALIAS}.event_count > 10"]
        sql = "select user_id, event_date, event_count from {{ source_table }}"

    executor.run(DeleteInsertTransform(), template_context={"source_table": src_table.qualified_name})

    tgt_table.assert_table_equals(
        [
            {"user_id": 1, "event_date": date(2024, 1, 1), "event_count": 5},
            {"user_id": 1, "event_date": date(2024, 1, 2), "event_count": 16},
            {"user_id": 2, "event_date": date(2024, 1, 1), "event_count": 8},
            {"user_id": 2, "event_date": date(2024, 1, 2), "event_count": 26},
        ],
        order_by=["user_id", "event_date"],
    )


def test_delete_insert_target_table_does_not_exist(
    conn: pyathena.connection.Connection, adapter: AthenaAdapter, executor: Executor, src_table: Table, tgt_table: Table
) -> None:
    cursor = conn.cursor()
    cursor.execute(f"drop table if exists {tgt_table.qualified_name}")
    src_table.insert([(1, date(2024, 1, 1), 10), (2, date(2024, 1, 2), 20)])

    class DeleteInsertTransform(IncrementalTableTransformation):
        target_schema = tgt_table.schema
        target_table = tgt_table.name
        strategy = "delete_insert"
        unique_key = "user_id"
        sql = "select user_id, event_date, event_count from {{ source_table }}"

    executor.run(DeleteInsertTransform(), template_context={"source_table": src_table.qualified_name})

    tgt_table.assert_table_equals(
        [
            {"user_id": 1, "event_date": date(2024, 1, 1), "event_count": 10},
            {"user_id": 2, "event_date": date(2024, 1, 2), "event_count": 20},
        ],
        order_by=["user_id"],
    )

    assert not adapter.table_exists(tgt_table.schema, f"{tgt_table.name}_tmp")


def test_delete_insert_without_unique_key_raises_error(executor: Executor, schema: str, src_table: Table) -> None:
    src_table.insert([(1, date(2024, 1, 1), 10), (2, date(2024, 1, 2), 20)])

    class DeleteInsertTransform(IncrementalTableTransformation):
        target_schema = schema
        target_table = "bad_incremental"
        strategy = "delete_insert"
        sql = "select user_id, event_date, event_count from {{ source_table }}"

    with pytest.raises(ValueError, match="unique_key is required for delete_insert materialization"):
        executor.run(DeleteInsertTransform(), template_context={"source_table": src_table.qualified_name})


def test_merge_single_unique_key(
    conn: pyathena.connection.Connection, executor: Executor, registry: SchemaRegistry, src_table: Table, tgt_table: Table
) -> None:
    tgt_table.insert([(1, date(2024, 1, 1), 10), (2, date(2024, 1, 1), 20)])

    class MergeTransform(IncrementalTableTransformation):
        target_schema = tgt_table.schema
        target_table = tgt_table.name
        strategy = "merge"
        unique_key = "user_id"
        sql = "select user_id, max(event_date) as event_date, sum(event_count) as event_count from {{ source_table }} group by user_id"

    src_table.insert([(2, date(2024, 1, 2), 25), (3, date(2024, 1, 3), 30)])
    executor.run(MergeTransform(), template_context={"source_table": src_table.qualified_name})

    tgt_table.assert_table_equals(
        [
            {"user_id": 1, "event_date": date(2024, 1, 1), "event_count": 10},
            {"user_id": 2, "event_date": date(2024, 1, 2), "event_count": 25},
            {"user_id": 3, "event_date": date(2024, 1, 3), "event_count": 30},
        ],
        order_by=["user_id"],
    )

    src_table.delete()
    src_table.insert([(3, date(2024, 1, 3), 35), (4, date(2024, 1, 4), 40)])
    executor.run(MergeTransform(), template_context={"source_table": src_table.qualified_name})

    tgt_table.assert_table_equals(
        [
            {"user_id": 1, "event_date": date(2024, 1, 1), "event_count": 10},
            {"user_id": 2, "event_date": date(2024, 1, 2), "event_count": 25},
            {"user_id": 3, "event_date": date(2024, 1, 3), "event_count": 35},
            {"user_id": 4, "event_date": date(2024, 1, 4), "event_count": 40},
        ],
        order_by=["user_id"],
    )


def test_merge_composite_unique_key(
    conn: pyathena.connection.Connection, executor: Executor, registry: SchemaRegistry, src_table: Table, tgt_table: Table
) -> None:
    tgt_table.insert([(1, date(2024, 1, 1), 10), (2, date(2024, 1, 1), 20), (1, date(2024, 1, 2), 15), (2, date(2024, 1, 2), 25)])
    src_table.insert([(1, date(2024, 1, 2), 16), (2, date(2024, 1, 2), 26), (1, date(2024, 1, 3), 30), (2, date(2024, 1, 3), 35)])

    class MergeTransform(IncrementalTableTransformation):
        target_schema = tgt_table.schema
        target_table = tgt_table.name
        strategy = "merge"
        unique_key = ["user_id", "event_date"]
        sql = "select user_id, event_date, event_count from {{ source_table }}"

    executor.run(MergeTransform(), template_context={"source_table": src_table.qualified_name})

    tgt_table.assert_table_equals(
        [
            {"user_id": 1, "event_date": date(2024, 1, 1), "event_count": 10},
            {"user_id": 1, "event_date": date(2024, 1, 2), "event_count": 16},
            {"user_id": 1, "event_date": date(2024, 1, 3), "event_count": 30},
            {"user_id": 2, "event_date": date(2024, 1, 1), "event_count": 20},
            {"user_id": 2, "event_date": date(2024, 1, 2), "event_count": 26},
            {"user_id": 2, "event_date": date(2024, 1, 3), "event_count": 35},
        ],
        order_by=["user_id", "event_date"],
    )

    src_table.delete()
    src_table.insert([(1, date(2024, 1, 3), 31), (2, date(2024, 1, 3), 36), (1, date(2024, 1, 4), 40), (2, date(2024, 1, 4), 45)])
    executor.run(MergeTransform(), template_context={"source_table": src_table.qualified_name})

    tgt_table.assert_table_equals(
        [
            {"user_id": 1, "event_date": date(2024, 1, 1), "event_count": 10},
            {"user_id": 1, "event_date": date(2024, 1, 2), "event_count": 16},
            {"user_id": 1, "event_date": date(2024, 1, 3), "event_count": 31},
            {"user_id": 1, "event_date": date(2024, 1, 4), "event_count": 40},
            {"user_id": 2, "event_date": date(2024, 1, 1), "event_count": 20},
            {"user_id": 2, "event_date": date(2024, 1, 2), "event_count": 26},
            {"user_id": 2, "event_date": date(2024, 1, 3), "event_count": 36},
            {"user_id": 2, "event_date": date(2024, 1, 4), "event_count": 45},
        ],
        order_by=["user_id", "event_date"],
    )


def test_merge_with_incremental_predicates_single_string(
    conn: pyathena.connection.Connection, executor: Executor, registry: SchemaRegistry, src_table: Table, tgt_table: Table
) -> None:
    src_table.insert([(2, date(2024, 1, 2), 25), (3, date(2024, 1, 2), 30)])
    tgt_table.insert([(1, date(2024, 1, 1), 10), (2, date(2024, 1, 1), 16), (3, date(2024, 1, 1), 5)])

    class MergeTransform(IncrementalTableTransformation):
        target_schema = tgt_table.schema
        target_table = tgt_table.name
        strategy = "merge"
        unique_key = "user_id"
        incremental_predicates = f"{TARGET_TABLE_ALIAS}.event_count > 15"
        sql = "select user_id, event_date, event_count from {{ source_table }}"

    executor.run(MergeTransform(), template_context={"source_table": src_table.qualified_name})

    tgt_table.assert_table_equals(
        [
            {"user_id": 1, "event_date": date(2024, 1, 1), "event_count": 10},
            {"user_id": 2, "event_date": date(2024, 1, 2), "event_count": 25},
            {"user_id": 3, "event_date": date(2024, 1, 1), "event_count": 5},
            {"user_id": 3, "event_date": date(2024, 1, 2), "event_count": 30},
        ],
        order_by=["user_id", "event_date"],
    )


def test_merge_with_incremental_predicates_list(
    conn: pyathena.connection.Connection, executor: Executor, registry: SchemaRegistry, src_table: Table, tgt_table: Table
) -> None:
    src_table.insert([(1, date(2024, 1, 2), 16), (2, date(2024, 1, 2), 26)])
    tgt_table.insert([(1, date(2024, 1, 1), 5), (1, date(2024, 1, 2), 15), (2, date(2024, 1, 1), 8), (2, date(2024, 1, 2), 15)])

    class MergeTransform(IncrementalTableTransformation):
        target_schema = tgt_table.schema
        target_table = tgt_table.name
        strategy = "merge"
        unique_key = ["user_id", "event_date"]
        incremental_predicates = [f"{TARGET_TABLE_ALIAS}.event_date >= date '2024-01-02'", f"{TARGET_TABLE_ALIAS}.event_count > 10"]
        sql = "select user_id, event_date, event_count from {{ source_table }}"

    executor.run(MergeTransform(), template_context={"source_table": src_table.qualified_name})

    tgt_table.assert_table_equals(
        [
            {"user_id": 1, "event_date": date(2024, 1, 1), "event_count": 5},
            {"user_id": 1, "event_date": date(2024, 1, 2), "event_count": 16},
            {"user_id": 2, "event_date": date(2024, 1, 1), "event_count": 8},
            {"user_id": 2, "event_date": date(2024, 1, 2), "event_count": 26},
        ],
        order_by=["user_id", "event_date"],
    )


def test_merge_target_table_does_not_exist(
    conn: pyathena.connection.Connection, adapter: AthenaAdapter, executor: Executor, src_table: Table, tgt_table: Table
) -> None:
    src_table.insert([(1, date(2024, 1, 1), 10), (2, date(2024, 1, 2), 20)])

    class MergeTransform(IncrementalTableTransformation):
        target_schema = tgt_table.schema
        target_table = tgt_table.name
        strategy = "merge"
        unique_key = "user_id"
        sql = "select user_id, event_date, event_count from {{ source_table }}"

    executor.run(MergeTransform(), template_context={"source_table": src_table.qualified_name})

    tgt_table.assert_table_equals(
        [
            {"user_id": 1, "event_date": date(2024, 1, 1), "event_count": 10},
            {"user_id": 2, "event_date": date(2024, 1, 2), "event_count": 20},
        ],
        order_by=["user_id"],
    )

    assert not adapter.table_exists(tgt_table.schema, f"{tgt_table.name}_tmp")


def test_merge_without_unique_key_raises_error(executor: Executor, src_table: Table) -> None:
    src_table.insert([(1, date(2024, 1, 1), 10), (2, date(2024, 1, 2), 20)])

    class MergeTransform(IncrementalTableTransformation):
        target_schema = src_table.schema
        target_table = "bad_merge"
        strategy = "merge"
        sql = "select user_id, event_date, event_count from {{ source_table }}"

    with pytest.raises(ValueError, match="unique_key is required for merge materialization"):
        executor.run(MergeTransform(), template_context={"source_table": src_table.qualified_name})
